...
export class PricesComponent {
  ...
  fetchPricesList(settings: any = this.lastUsedSettings) {
    ...
    this.pricesService.searchPrices(body).pipe(
      tap(resp => {
        this.pricesList = resp?.data || [];
        this.totalRecords = resp?.totalRecords || 0;
        ({ noRecords: this.noRecords, noResults: this.noResults } = this.tableHelper.isNoResultsOrNoRecords(this.firstInit, this.totalRecords));
      }),
      catchError(error => {
  ...
  deletePrice(price: Price) {
    this.pricesService.deletePrice(price.id).pipe(
      tap(() => {
